...
class UserPollItemRepository
{
    ...
    public function hasVoted(int $user_id, int $poll_id): bool
    {
        $stmt = Mysql::getInstance()->getPdo()->prepare(
            'SELECT upi.* FROM user_poll_item upi JOIN poll_item pi ON upi.poll_item_id = pi.id WHERE upi.user_id = ? AND pi.poll_id = ?'
        );
    ...
    {
        $stmt = Mysql::getInstance()->getPdo()->prepare(
            'SELECT pi.id, pi.name, COUNT(upi.user_id) as count FROM poll_item pi LEFT JOIN user_poll_item upi ON pi.id = upi.poll_item_id WHERE pi.poll_id = ? GROUP BY pi.id, pi.name'
        );
    ...
    {
        $stmt = Mysql::getInstance()->getPdo()->prepare(
            'DELETE upi FROM user_poll_item upi INNER JOIN poll_item pi ON upi.poll_item_id = pi.id WHERE upi.user_id = ? AND pi.poll_id = ?'
        );
        $stmt->execute([$user_id, $poll_id]);
    }
}
